...
**Status update (2026-07-10): partially resolved.** M3-B3 (chat) and M3-B5 (playbook execution) shipped as in-Word surfaces on the `feature/word-addin-surfaces` branch, with honest deltas from the original task specs: chat is non-streaming (single JSON response via `MessageCreate.file_ids` snapshot attach) and does not yet render the 5-state Citation Engine UI or in-doc span highlighting; playbook execution applies redlines in one pass after the execution reaches a terminal state (no per-position SSE progress, no cost-preview confirmation), degrading unsearchable clauses to comments/missed tallies. Both depend on DOCX ingest (ADR 0017) being live in the deployment, and the manifest now requires WordApi 1.4. M3-B4 (skills tab) and M3-B6 (tier badge) remain open — the skills tab still renders a deep-link card.
