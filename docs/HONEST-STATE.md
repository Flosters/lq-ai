...
| Ingest formats — **PDF** (PyMuPDF/Docling), **plain text / Markdown** (`parse_text`, verbatim canonical text → exact-match citations; non-UTF-8 → `decode_error`), and **DOCX** (`parse_docx`, ADR 0017: Pandoc canonical stream; tracked changes resolved to the changes-accepted text with the insertion/deletion/comment revision layer retained in `structured_content`). Honest caveats: a comment anchored to an unaccepted insertion is dropped by Pandoc itself (upstream #9833 — OOXML fallback is DE-386), deletions/comments are not yet separately-citable chunks, and comment reply threads are not reconstructed. | M1 (PDF); post-v0.5.0 (text/md, DE-332); post-v0.6.1 (DOCX, ADR 0017/DE-386) | `api/app/pipeline/parsers.py`; `api/app/pipeline/ingest.py` |
